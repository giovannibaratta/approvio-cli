...
export async function assignUserRolesTask(userId: string, options: {roles: string}) {
  const client = getUserClient()
  const roles = parseRoles(options.roles)

  await handleTask(client.assignUserRoles(userId, {roles}), onSuccess, "Failed to assign roles")
}
